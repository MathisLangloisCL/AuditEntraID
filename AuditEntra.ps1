...
Get-EntraTierZeroServicePrincipals -Token $MSGraphToken | %{
    $tierZeroPrivilege = $_tiersZeroPrivilege # Stockage des privilèges Tier Zero associés à l'identité actuelle
    
    # Récupération des informations détaillées pour chaque Service Principal identifié
    Get-EntraServicePrincipal `
        -Token $MSGraphToken `
        -ObjectID $_ServicePrincipalID |      
        Select-Object id,                      
            appDisplayName,                    
            appOwnerOrganizationId,           
            @{Name="InternalApplication"; Expression={ $_appOwnerOrganizationId -eq "TenantID" }}, # Identifiant unique du tenant Azure (remplacer 'TenantID' par la valeur réelle)
            @{Name="TierZeroPrivilege"; Expression={ $tierZeroPrivilege }}
} | Export-Csv -Path "Serviceprincipalscsv" -NoTypeInformation -Encoding UTF8
# Exporte les informations collectées dans un fichier CSV appelé "Serviceprincipalscsv"
